...
from src.models.video_simplergb import Multi_DMC,DMC
...
parser.add_argument('--model_name', type=str, default="finetune3_8", help='models_name')
parser.add_argument("--i_model", type=str, default='./cvpr2023_image_psnr.pth.tar')
parser.add_argument('--gop_size', type=int,default=50)
...
parser.add_argument('--test_class', type=str, default="DJL", help='Path to save models')
        ...
        # for frame_idx in tqdm(range(40)):
            with torch.no_grad():
                ...
                if frame_idx % args.gop_size == 0:
                    ...
                    ##执行I帧分辨率下采样
                    down_factor_i = 1/2
                    cur_frame_resize = F.interpolate(cur_frame,scale_factor=down_factor_i,mode="bilinear")
                    padding_l_d, padding_r_d, padding_t_d, padding_b_d = get_padding_size(int(h*down_factor_i), int(w*down_factor_i), 64)
                    ...
                    recon_frame =  F.interpolate(recon_frame ,scale_factor=1/down_factor_i,mode="bilinear")
                    ...
                    cnt +=1
                    ### 为p帧参考做形变
                    ref_frame =  F.interpolate(recon_frame,scale_factor=down_factor,mode="bilinear")
                ...
                cur_frame_resize_pad = torch.nn.functional.pad(
                            ...
                            (padding_l_d, padding_r_d, padding_t_d, padding_b_d),
                            mode="replicate",)
                result = video_net.forward_one_frame(cur_frame_resize_pad,dpb,idx_,True)               
                if frame_idx >0 :
               ...
               #************指标计算**************
                recon_frame = F.pad(ref_frame, (-padding_l_d, -padding_r_d, -padding_t_d, -padding_b_d), mode="replicate",)
...
def eval():
    ...
    test_dataset = CTS_yuv("/mnt/data-ssd2/shliao/DATA",f"{args.test_class}",output_rgb=True)
    # test_dataset = CTS("/mnt/data-ssd2/shliao/DATA",f"{args.test_class}",output_yuv=False)
    ...
    # p_model_path = os.path.join(args.p_model, args.model_name)+".pth"
