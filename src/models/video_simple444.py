...
class DMC(CompressionModel):
    ...
    def get_q_for_inference(self,  q_index):
        ...
        return vector_enc,vector_dec,vector_rec,vector_tem,vector_hyper1,vector_hyper2
        
    def forward_one_frame(self, x, dpb, frame_idx,use_lpip):
        _, _, H, W = x.size()
        pixel_num = H * W
        vector_enc,vector_dec,vector_rec,vector_tem,vector_hyper1, vector_hyper2 = self.get_q_for_inference(frame_idx)
        ...
        y_loss = torch.mean((x_rec[:,0:1,] -x[:,0:1,]).pow(2))
        u_loss = torch.mean((x_rec[:,1:2,] -x[:,1:2,]).pow(2))
        v_loss = torch.mean((x_rec[:,2:3,] -x[:,2:3,]).pow(2))
        ...
        if use_lpip:
            rec_rgb = ycbcr2rgb(x_rec).clamp(0.0, 1.0)
            rgb  = ycbcr2rgb(x).clamp(0.0, 1.0)
            lpips_loss = self.lpips_fn(rec_rgb, rgb, normalize=True).mean()
            ssim_loss = 1-ms_ssim(rec_rgb, rgb,data_range=1.0).mean()
    ...
    def cal_macs(self):
        ...
        vector_enc,vector_dec,vector_rec,vector_tem,vector_hyper1, vector_hyper2 =  self.get_q_for_inference(0)
