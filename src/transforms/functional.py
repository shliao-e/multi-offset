...
#     rgb = np.clip(rgb, 0., 1.)
#     return rgb
def ycbcr420_to_rgb(y, u, v):
    """
    PyTorch实现的YUV420转RGB
    """
    # 上采样UV
    u_up = F.interpolate(u, size=y.shape[-2:], mode='bilinear')
    v_up = F.interpolate(v, size=y.shape[-2:], mode='bilinear')
    
    # BT.709转换
    r = y + 1.5748 * (v_up - 0.5)
    g = y - 0.1873 * (u_up - 0.5) - 0.4681 * (v_up - 0.5)
    b = y + 1.8556 * (u_up - 0.5)
...
def ycbcr_to_rgb(yuv):
    '''
    yuv is Bx3xhxw YCbCr float tensor, in the range of [0, 1]
    '''
    y, cb, cr = torch.chunk(yuv,3,1)
    Kr, Kg, Kb = YCBCR_WEIGHTS["ITU-R_BT.709"]
    r = y + (2 - 2 * Kr) * (cr - 0.5)
    b = y + (2 - 2 * Kb) * (cb - 0.5)
    g = (y - Kr * r - Kb * b) / Kg
