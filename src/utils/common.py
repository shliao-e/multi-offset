...
def flops_calculator(pixels, model, *args):
    device = next(model.parameters()).device
    args = [arg.to(device) for arg in args]
    macs, params = profile(model, inputs=(*args, ))
    macs, params = clever_format([macs / pixels, params], '%.3f')
    return macs
...
def get_loss_info(rd):
    ...
    if rd['bpp_y'] is not None:
        info = {
            "bpp_y": rd['bpp_y'].item(),
            "bpp_z": rd['bpp_z'].item(),
            "bpp_offset": rd['bpp_offset'].item(),
            "bpp": rd['bpp'].item(),
            "mse_loss": rd['mse_loss'].item(),
        'lpips_loss':None,
        }
    if  rd['lpips_loss'] is not None:
        info.update({"lpips_loss": rd['lpips_loss'].item()})
        info.update({"ssim_loss": rd['ssim_loss'].item()})
        
    return info
def get_rd_info(result):
    
    if result['bpp_y'] is not None:
        bpp_y = torch.mean(result['bpp_y']) 
        bpp_z = torch.mean(result['bpp_z']) 
        bpp_offset = torch.mean(result['bpp_offset']) 
        bpp = torch.mean(result['bpp']) 
    mse_loss = torch.mean(result['mse_loss']) 
    if "lpips_loss" in result:
        lpips_loss = result['lpips_loss']
