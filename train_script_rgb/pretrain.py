...
from src.models.video_simplergb import DMC
...
from src.utils.tools import load_models,train_init

parser = argparse.ArgumentParser(description="Example training script")
parser.add_argument('--cuda_id', type=int, default= 2, help='Use cuda')
...
parser.add_argument('--save_dir', type=str, default="./Argb_checkpoints/pretrain/", help='Path to save models')
...
training_strategy = {
    'pretrain': [[ 1e-4,  "residue", "recon_mse",           2]]     * 3 +\
                [[ 1e-4,  "residue", "recon_mse",           3]]     * 3 +\
                [[ 1e-4,  "residue", "recon_mse",           4]]     * 3 +\
                [[ 1e-4,  "residue", "recon_mse",           5]]     * 3 +\
                [[ 2e-4,  "residue", "total_rdc_mse",       2]]     * 4 +\
                [[ 2e-4,  "residue", "total_rdc_mse",       3]]     * 4 +\
                [[ 2e-4,  "residue", "total_rdc_mse",       4]]     * 4 +\
                [[ 2e-4,  "residue", "total_rdc_mse",       5]]     * 4 +\
                [[ 2e-4,  "residue", "total_rdc_mse",       6]]     * 4+\
                [[ 1e-4,  "all",     "total_rdc_mse",       7]]     * 4 +\
                [[ 5e-5,  "all",     "total_rdc_mse",       7]]     * 4 +\
                [[ 1e-5,  "all",     "total_rdc_mse",       7]]     * 4 +\
                [[ 5e-6,  "all",     "total_rdc_mse",       7]]     * 4+\
                [[ 1e-6,  "all",     "total_rdc_mse",       7]]     * 3 
}
...
def train():
    ...
    for i in range(stage_name.index(start_stage),len(stage_name)):
        ...
        train_mode = "frame"
        logger.info(f"********** Current strategy: {current_stage}   |   Start_epoch: {start_epoch}   |**********")
